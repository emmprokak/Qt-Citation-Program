...
void ChapterCitationWindow::on_button_copy_clicked()
{
    ...
    QTimer::singleShot(0,[&](){ ui_4->label_copy->setText("Copied everything!");});
    QTimer::singleShot(1500,ui_4->label_copy,&QLabel::hide);
    QTimer::singleShot(1500,[&](){ ui_4->label_copy->setText(" ");});
...
void ChapterCitationWindow::format_citation()
{
    ...
    if(this->style == "Harvard Style")
        this->format += " (" + year_in + "), " + format_title(title_in) + ", " + place_in + ": " + publisher_in;
    else if(this->style == "APA Style")
        this->format += " (" + year + "). " + format_title(title_in) + ". " + place_in + ": " + publisher_in;
